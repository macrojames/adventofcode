...
def connect_network(p0, p1):
    found = {0:None, 1:None}
    if SAMPLE: print(f"Testing: {[p0, p1]}")
    for idx, n in enumerate(networks):
        ...
        if p1 in n:
            found[1] = idx
        if all([_ is not None for _ in found.values()]): break
    else:
            # kein Break oben, also eigenes Network
        if all([_ is None for _ in found.values()]):
            networks.append([p0, p1])
            if SAMPLE: print(f"New Network: {[p0, p1]}")
            return
    if found[0] != found[1] and all([_ is not None for _ in found.values()]):
            # join
        if SAMPLE: print(f"Joining Network: {networks[found[0]]}")
        networks[found[0]].extend(networks.pop(found[1]))
    else:
        if found[0] == found[1]:
            if SAMPLE: print(f"Skipped {p0=} and {p1=} in Network: {networks[found[0]]}")
        elif found[0] is not None:
            networks[found[0]].append(p1)
            if SAMPLE: print(f"Added {p1=} Network: {networks[found[0]]}")
        elif found[1] is not None:
            if SAMPLE: print(f"Added {p0=} Network: {networks[found[1]]}")
...
def part2():
    ...
    for _, (p0, p1) in sorted_distances:
        connect_network(p0, p1)
        i += 1
        if SAMPLE and i % 20: print("Iteration:", i)
